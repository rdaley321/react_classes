...
class Factory {
  ...
  static customerBuild(color, options) {
    console.log("Building " + color.color + " " + color.trim + " " + color.model + " with the following options: " + options)
  }
  constructor() {
    this.make = "Mazda"
    this.location = "USA"
...
Factory.customerBuild(mazda3, ["weather package", " satellite radio", " rear spoiler"])
...
Factory.customerBuild(miataRf, ["hid headlights", " sports suspension", " leather steering wheel", " heated seats", " adaptive cruise control"])
...
Factory.customerBuild(trailBlazer, ["seat warmers", " tinted windows", " fog lamps"])
